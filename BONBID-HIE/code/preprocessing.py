...
def get_transforms(prob_brightness=0.0, prob_elastic=0.0, is_train=True, is_aug=True):
    transforms = [
    ...
    if is_train and is_aug:
        transforms += [
            RandBrightnessd(keys=["image"], prob=prob_brightness),
            # Rand3DElasticd(keys=["image"], sigma_range=(2, 2), magnitude_range=(100, 200), prob=prob_elastic, spatial_size=(256, 256, 64))
        ]
    return Compose(transforms)

## 학습 할 때 사용
def create_and_preprocess_dataset(adc_path, z_adc_path, label_path, prob_elastic=0.0, is_train=True, is_aug=True):
    data_paths = combine_paths_to_dic(adc_path, z_adc_path, label_path)
    transforms = get_transforms(prob_elastic=prob_elastic, is_train=is_train, is_aug=is_aug)
    dataset = Dataset(data=data_paths, transform=transforms)
    return dataset

# # Grid Search 할 때 사용
def create_combined_path(adc_path, z_adc_path, label_path):
    data_paths = combine_paths_to_dic(adc_path, z_adc_path, label_path)
    return data_paths

def preprocess_dataset(data_paths, prob_brightness=0.0, prob_elastic=0.0, is_train=True, is_aug=True):
    transforms = get_transforms(prob_brightness=prob_brightness, prob_elastic=prob_elastic, is_train=is_train, is_aug=is_aug)
    dataset = Dataset(data=data_paths, transform=transforms)
    return dataset
