...
class Unet3DModel:
    ...
    def Unet_model(self):
        ...
        dice_metric_and_loss = DiceMetricAndLoss()

        model.compile(optimizer='adam',
                    loss=dice_metric_and_loss.compute_loss,  # 손실 함수
                    metrics=['accuracy', MeanIoU(num_classes=2), dice_metric_and_loss])  # Dice Coefficient 메트릭
    ...
    def train(self, train_tf_dataloader, val_tf_dataloader, batch_size=1, epochs=100):
        ...
        checkpoint_dice = ModelCheckpoint(checkpoint_path, 
                                  monitor='val_dice_metric_and_loss',
                                  verbose=1,
                                  save_best_only=True, 
                                  mode='max')
    ...
    def plot_prob_performance(self, prob_history, augmentations):
        if augmentations == 'both':
            probs_brightness, probs_elastic, scores = zip(*prob_history)
            plt.scatter(probs_brightness, probs_elastic, c=scores, cmap='viridis', marker='o')
            plt.colorbar(label='Validation Dice Coefficient')
            plt.xlabel('RandAdjustBrightnessd Prob Value')
            plt.ylabel('Rand3DElasticd Prob Value')
        else:
            probs, scores = zip(*prob_history)
            plt.scatter(probs, scores, cmap='viridis', marker='o')
            plt.colorbar(label='Validation Dice Coefficient')
            plt.xlabel(f'RandAdjust{augmentations.capitalize()}d Prob Value')
    ...
    def plot_epoch_performance(self, all_histories, augmentations):
        # 모든 prob 값에 대한 성능 변화 그래프 그리기
        plt.figure(figsize=(12, 8))
        for prob_key, history in all_histories.items():
            if augmentations == 'both':
                prob_brightness, prob_elastic = prob_key
                label = f'Prob brightness: {prob_brightness}, Prob elastic: {prob_elastic}'
            else:
                prob = prob_key
                label = f'Prob {augmentations}: {prob}'

            plt.plot(range(1, len(history) + 1), history, label=label)

        plt.xlabel('Epoch')
        plt.ylabel('Validation Dice Coefficient')
        plt.title(f'Validation Dice Coefficient Across Epochs for Different {augmentations.capitalize()} Prob Values')
        plt.legend()
        plt.grid(True)
        plt.show()


    def grid_search_prob_tuning(self, train_path, val_path, param_grid, epochs=100, augmentations=None):
        ...
        for params in param_list:
            # 증강 방식에 따라 다르게 처리
            prob_brightness = params.get('prob_brightness', None)
            prob_elastic = params.get('prob_elastic', None)

            if augmentations == 'brightness':
                print(f"Training with prob_brightness={prob_brightness}")
                train_dataset = preprocess_dataset(train_path, prob_brightness=prob_brightness, is_train=True)
                val_dataset = preprocess_dataset(val_path, prob_brightness=prob_brightness, is_train=False, is_aug=False)
            elif augmentations == 'elastic':
                print(f"Training with prob_elastic={prob_elastic}")
                train_dataset = preprocess_dataset(train_path, prob_elastic=prob_elastic, is_train=True)
                val_dataset = preprocess_dataset(val_path, prob_elastic=prob_elastic, is_train=False, is_aug=False)
            elif augmentations == 'both':
                print(f"Training with prob_brightness={prob_brightness}, prob_elastic={prob_elastic}")
                train_dataset = preprocess_dataset(train_path, prob_brightness=prob_brightness, prob_elastic=prob_elastic, is_train=True)
                val_dataset = preprocess_dataset(val_path, prob_brightness=prob_brightness, prob_elastic=prob_elastic, is_train=False, is_aug=False)
            ...
            # prob 값과 해당 성능을 저장
            prob_history.append((prob_brightness, prob_elastic, val_dice_coefficient) if augmentations == 'both' else (prob_brightness or prob_elastic, val_dice_coefficient))

            # 각 prob 값에 대한 모든 에포크의 성능 기록
            all_histories[(prob_brightness, prob_elastic) if augmentations == 'both' else (prob_brightness or prob_elastic)] = history.history['val_dice_coefficient']

            if val_dice_coefficient > best_score:
                best_score = val_dice_coefficient
                best_params = params

        self.plot_prob_performance(prob_history, augmentations)
        self.plot_epoch_performance(all_histories, augmentations)
